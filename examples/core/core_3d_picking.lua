        ...
        Window.BeginDrawing();
            ...
            Window.BeginMode3D(camera);
                ...
                if (collision.hit) then
                    Graphics3D.DrawCube(cubePosition, cubeSize.x, cubeSize.y, cubeSize.z, COLOR_RED);
                    Graphics3D.DrawCubeWires(cubePosition, cubeSize.x, cubeSize.y, cubeSize.z, COLOR_MAROON);
                    Graphics3D.DrawCubeWires(cubePosition, cubeSize.x + 0.2, cubeSize.y + 0.2, cubeSize.z + 0.2, COLOR_GREEN);
                else
                    Graphics3D.DrawCube(cubePosition, cubeSize.x, cubeSize.y, cubeSize.z, COLOR_GRAY);
                    Graphics3D.DrawCubeWires(cubePosition, cubeSize.x, cubeSize.y, cubeSize.z, COLOR_DARKGRAY);
                end

                Graphics3D.DrawRay(ray, COLOR_MAROON);
                Graphics3D.DrawGrid(10, 1.0);

            Window.EndMode3D();

            Graphics2D.DrawText("Try clicking on the box with your mouse!", 240, 10, 20, COLOR_DARKGRAY);

            if (collision.hit) then
                Graphics2D.DrawText("BOX SELECTED", (screenWidth - Text.Measure("BOX SELECTED", 30)) / 2, (screenHeight * 0.1), 30, COLOR_GREEN);
            end

            Graphics2D.DrawText("Right click mouse to toggle camera controls", 10, 430, 10, COLOR_GRAY);
